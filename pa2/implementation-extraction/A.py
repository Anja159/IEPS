...
def extractRegex(pages):
    ...
    for page in pages:
        ...
        if 'overstock' in parsed_page:
            regex = r'<b>([0-9].+)<\/b>.*\n.*\n.*<s>([\$0-9,.]+).*\n.*<b>([\$0-9.]+).*\n.*>([\$0-9.,]+)\s.([0-9]+%)' \
                    r'.*\n.*\n.*<span class=\"normal\">(.*|.*\n.*\n.*|.*\n.*\n.*\n.*)<br\/><a'
            params = ['Title', 'ListPrice', 'Price', 'Saving', 'SavingPercent', 'Content']
            ...
            params = ['Title', 'Subtitle', 'Lead', 'Author', 'PublishTime', 'Content']
        ...
        for match in matches:
            ...
            for i, param in enumerate(params):
                if param == 'Content' and 'rtvslo' in parsed_page:
                    content = '\n'.join(line.strip() for line in BeautifulSoup(match.group(i+1), 'html.parser').get_text(separator='\n').split('\n') if line.strip())
